...
function createPDF($info, $products_info, $contains22) {
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(-1);
            $this->Cell(0, 12, "Yesha Enterprises", 0, 1, 'C');
            // Address and contact information
            $this->SetFont('Arial', '', 10);
            $this->Cell(-1);
            $this->Cell(0, 8, "71 Govind Sarang Vyavasiyak Parisar, New Rajendra Nagar,Raipur,Chhatisghar ZIP - 492001.", 0, 1, 'C');
            $this->Cell(-1);
            $this->Cell(0, 8, "PH : 9770403382, 9977228924, Email Id: yeshaenterprises01@gmail.com", 0, 1, 'C');
            $this->Cell(-1);
            $this->Cell(0, 8, "GSTIN: 22GEIPP1287J1Z6 , State: 22-Chhattisgarh", 0, 1, 'C');
        }
        ...
        function body($products_info)
        {
            ...
            $this->SetX(150);
            $this->SetFont('Arial', 'B', 14);
            $this->Cell(50, 1, "ESTIMATE No:" . $info["ESTIMATE_no"], 0, 0, 'R');
            $this->SetX(150);
            $this->Cell(50, 20, "ESTIMATE Date:".  $info["ESTIMATE_date"], 0, 0, 'R');
           // Table Header
            $this->SetY(105);
...
$sql = "SELECT * FROM estimate WHERE SID='$id'";
$res = $conn->query($sql);
...
if ($res->num_rows > 0) {
    ...
    $info = [
        ...
        "city" => $row["CCITY"],
        "ESTIMATE_no" => $row["EST_NO"],
        "gstin" => $row["GSTIN"],
        "ESTIMATE_date" => date("d-m-Y", strtotime($row["EST_DATE"])),
    ...
    $products_info = [];
    $sql = "SELECT * FROM estimate_products WHERE SID='$id'";
    $res = $conn->query($sql);
